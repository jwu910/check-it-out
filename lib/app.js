...
var git = require(path.resolve(__dirname, 'utils/git'));
var dialogue = require(path.resolve(__dirname, 'utils/interface'));

var _require = require(path.resolve(__dirname, 'utils/utils')),
    getRemoteTabs = _require.getRemoteTabs;
...
var start = exports.start = function start(args) {
  ...
  screen.key('r', function () {
    git.doFetchBranches().then(function () {
  ...
  var handleSuccess = function handleSuccess(success, branch, type) {
    screen.destroy();

    process.stdout.write(chalk.bold.green('[SUCCESS] ') + type + ' ' + chalk.yellow(branch) + '\n');
  ...
  branchTable.on('select', function (selectedLine) {
    ...
    git.doCheckoutBranch(gitBranch, gitRemote).then(function (_ref) {
  ...
  function refreshTable(currentRemote) {
    var listArray = git.buildListArray(currentRemote);

    var branchArray = [];

    git.buildRemoteList().then(function (results) {
      remoteList = results;
