...
const { getRemoteTabs, readError } = require(path.resolve(__dirname, 'utils/utils'));
...
const start = exports.start = args => {
  ...
  branchTable.on('select', selectedLine => {
    ...
    return doCheckoutBranch(gitBranch, gitRemote).then(output => {
      screen.destroy();
